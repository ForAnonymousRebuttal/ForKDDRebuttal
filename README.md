...
- **operation 1 & 2:** $H = W_fXW_{h1} + b$, where $X \in \mathbb{R}^{F \times d}$ is the embedded input features of a sample, $W_{h1} \in \mathbb{R}^{d \times 2d^’}$ is transformation on the hidden dimension, $W_f \in \mathbb{R}^{F \times F}$ is transformation on feature dimension in SGU (Eq. 6), $b$ is the bias term combining the ones of $W_f$ and $ W_{h1}$;
- **operation 3 & 4:** $\hat{H} = H_{:, :d^’}  \odot H_{:, d^’:} W_{h2}$, where $\odot$ is Hadamard product and $W_{h2} \in \mathbb{R}^{d^’ \times d}$ is transformation on hidden dimension, $\hat{H}$ is used for final prediction. 
